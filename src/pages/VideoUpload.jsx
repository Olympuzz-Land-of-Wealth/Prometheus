...
export default function VideoUpload() {
  ...
  const handleFile = useCallback(async (file) => {
    ...
    try {
      ...
      setCurrentStep(3);
      await waitForAnalysis(session_id);
      setCurrentStep(4);
      await new Promise((r) => setTimeout(r, 600));
      navigate('/dashboard', { state: { session_id } });
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col items-center justify-center p-8 bg-prometheus-bg font-inter">
      ...
      >
        <div className="bg-prometheus-card border border-prometheus-border rounded-lg p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-prometheus-secondary text-[13px] mt-1">
              Upload a video file to begin AI-powered detection
            </p>
          </div>
            ...
            <AnimatePresence mode="wait">
              {!fileName ? (
                <motion.div key="upload" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center">
                  ...
                  <p className="text-prometheus-cream text-[14px] font-medium">Drop video file or click to browse</p>
                  <p className="text-prometheus-secondary text-[12px] mt-1">Supports MP4, MOV, AVI</p>
                </motion.div>
              ) : (
                <motion.div key="uploaded" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col items-center">
        ...
        <motion.div key={currentStep} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="mt-5 text-center">
          <p className="text-prometheus-secondary text-[13px]">{STATUS_MESSAGES[currentStep]}</p>

...
1. Download the `best.pt` weights from the project's shared Google Drive.
...
Prometheus is built with **Privacy by Design**. Any data transmitted for storage MUST be processed through the `pre-processing.py` script to ensure faces are blurred.
